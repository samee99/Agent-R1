...
def compute_gae_advantage_return(token_level_rewards: torch.Tensor, values: torch.Tensor, action_mask: torch.Tensor,
...
def compute_policy_loss(old_log_prob, log_prob, advantages, action_mask, cliprange):
    ...
    pg_loss = verl_F.masked_mean(torch.max(pg_losses, pg_losses2), action_mask)
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses).float(), action_mask)
