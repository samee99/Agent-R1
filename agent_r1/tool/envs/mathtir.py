...
class MathTIREnv(BaseToolEnv):
    ...
    def process_responses_ids(self, tokenizer, raw_responses_ids: Tensor) -> Tensor:
        def process_response_ids(raw_response_ids):
            response_tokens = tokenizer.convert_ids_to_tokens(raw_response_ids)
            response_sofar = ""
            flag = False
            for i, token in enumerate(response_tokens):
                response_sofar += token
                if "```python" in response_sofar:
                    response_sofar = response_sofar.split("```python")[-1]
                    flag = True
                if flag and "```" in response_sofar:
